...
export function AllGamesList({ groupedGames, onGameClick }: AllGamesListProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {groupedGames.leagues.map((leagueGroup) => (
        <Card key={leagueGroup.league} className="overflow-hidden" data-testid={`league-card-${leagueGroup.league}`}>
          <CardHeader className="bg-muted/50 border-b">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Trophy className="h-5 w-5 text-primary" />
                <div>
                  <CardTitle className="text-xl" data-testid={`league-title-${leagueGroup.league}`}>
                    {leagueGroup.league}
                  </CardTitle>
